...
.controller('CarouselController', ['$scope', '$element', '$interval', '$animate', function ($scope, $element, $interval, $animate) {
  ...
  self.select = $scope.select = function(nextSlide, direction) {
    ...
    //Prevent this client-triggered transition from occurring if there is already one in progress
...
/**
 * A helper service that can parse typeahead's syntax (string provided by clients)
      ...
      link: function(originalScope, element, attrs, ctrls) {
        ...
        var getMatchesAsync = function(inputValue) {
          ...
          $q.when(parserResult.source(originalScope, locals)).then(function(matches) {
            //it might happen that several async queries were in progress if a client were typing fast
            ...
            if (onCurrentRequest && hasFocus) {
              if (matches && matches.length > 0) {
                ...
                //Select the single remaining option if client input matches
        ...
        //we need to propagate client's query so we can higlight matches
        ...
        modelCtrl.$parsers.unshift(function(inputValue) {
          ...
          if (isEditable) {
            ...
            if (!inputValue) {
              // Reset in case client had typed something previously.
  ...
  .filter('typeaheadHighlight', ['$sce', '$injector', '$log', function($sce, $injector, $log) {
    ...
    return function(matchItem, query) {
      if (!isSanitizePresent && containsHtml(matchItem)) {
        $log.warn('Unsafe use of typeahead please use ngSanitize'); // Warn the client about the danger
      }
